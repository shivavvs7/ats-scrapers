...
from join_com.api_client import JoinATSClient
...
def extract_company_slug(url: str) -> str:
    """Extract company slug from join.com URL."""
    parsed = urlparse(url)
    path = parsed.path.lstrip("/")
    # Path is like "companies/acbs-assurances"
...
def scrape_join_jobs(
    company_slug: str,
    force: bool = False,
    company_name: str = None,
    client: JoinATSClient = None,
):
    ...
    if not should_scrape:
        ...
        return file_path, num_jobs, False
    ...
    print(f"Fetching fresh data for {company_slug}...")

    if client is None:
        client = JoinATSClient()

    for attempt in range(1, MAX_RETRIES + 1):
        try:
            jobs = client.get_all_jobs(company_slug)
            ...
            return file_path, len(jobs), True

        except Exception as e:
            if attempt == MAX_RETRIES:
                print(f"Failed to scrape '{company_slug}' after {MAX_RETRIES} attempts: {e}")
                return None, 0, False
...
def scrape_all_join_jobs(force: bool = False):
    """Scrape all companies from join_companies.csv."""
    script_dir = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(script_dir, "join_companies.csv")
    ...
    for company_slug in companies:
        ...
        result, num_jobs, was_scraped = scrape_join_jobs(
            company_slug, force, company_name, client
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Scrape join.com job boards")
    parser.add_argument(
        "company_slug",
        nargs="?",
    ...
    try:
        if args.company_slug:
            scrape_join_jobs(args.company_slug, args.force)
        else:
            scrape_all_join_jobs(args.force)
    finally:
